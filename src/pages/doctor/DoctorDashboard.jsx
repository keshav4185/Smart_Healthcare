...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    doctorService.getDashboard()
      .then(data => {
        setStats({
          todayAppointments: data.appointments?.filter(a => a.date === new Date().toISOString().split('T')[0]).length || 0,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <Card title="⚡ Quick Actions">
            <div className="space-y-3">
              <Button variant="primary" className="w-full" onClick={() => navigate('/doctor/appointments')}>📅 View All Appointments</Button>
              <Button variant="secondary" className="w-full" onClick={() => navigate('/doctor/patients')}>👥 Manage Patients</Button>
              <Button variant="outline" className="w-full" onClick={() => navigate('/doctor/diagnoses')}>🩺 Pending Diagnoses</Button>
            </div>
          </Card>
